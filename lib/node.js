...
{
	const url = 'http://' + (process.env.RPC_HOST || 'localhost') + ':' + (process.env.RPC_PORT || '8545');
	console.info(`Starting web3 connection to ${url}`);
...
Node.prototype.startSocketConnection = function()
{
	if( !this._socket )
	{
		const wsServer = process.env.WS_SERVER || 'ws://localhost:3000';

		console.info('wsc', `Starting socket connection to ${wsServer}`);

		socket = new Socket( wsServer );
